...
async def get_conversation_history(project_id: int, db: AsyncSession, limit: int = 5) -> list:
    """
    Retrieve conversation history for a project to provide context to the LLM.

    Args:
        project_id: The project ID
        db: Database session
        limit: Maximum number of previous conversations to retrieve (default: 5)

    Returns:
        List of dicts with 'question' and 'sql' keys, ordered oldest to newest
    """
    try:
        stmt = (
        ...
        return [
            {"question": r.question, "sql": r.generated_sql}
...
async def generate_sql_from_question(
    ...
    Args:
        ...
        db_metadata: Database schema metadata
        dialect: SQL dialect (Oracle, PostgreSQL, etc.)
        conversation_history: Previous questions and SQL queries for context
    """
    ...
    try:
        ...
        if conversation_history and len(conversation_history) > 0:
            ...
            for idx, conv in enumerate(conversation_history, 1):
                ...
                if conv.get('sql'):
                    # Show first 150 chars of SQL to avoid token overflow
                    sql_preview = conv['sql'][:150]
                    if len(conv['sql']) > 150:
                        sql_preview += "..."
        ...
        logger.info(f"[SQL_GEN] Raw LLM response: {sql_query[:500]}")  # Log first 500 chars
        ...
        # Check for multiple SELECT statements at the TOP LEVEL only (not in subqueries)
        # Split by newlines and look for lines that start with SELECT
        lines = sql_query.split('\n')
        select_line_indices = [i for i, line in enumerate(lines) if re.match(r'^\s*SELECT\b', line, re.IGNORECASE)]

        if len(select_line_indices) > 1:
            # Check if these are separate queries or part of UNION/subquery
            # Look for UNION between the SELECT statements
            first_idx = select_line_indices[0]
            second_idx = select_line_indices[1]
            between_text = '\n'.join(lines[first_idx:second_idx])

            # If no UNION/FROM/WHERE/JOIN between them, they're separate queries
            if not re.search(r'\b(UNION|FROM|WHERE|JOIN)\b', between_text, re.IGNORECASE):
                # Take only lines up to the second SELECT
                sql_query = '\n'.join(lines[:second_idx]).strip()
                sql_query = re.sub(r'\s+(UNION\s*ALL?)\s*$', '', sql_query, flags=re.IGNORECASE).strip()
...
async def execute_query(
    ...
    try:
        ...
        # Step 2.5: Retrieve conversation history for context
        ...
        # Step 3: Generate SQL from question using LLM (with conversation context)
        ...
        # Step 8.5: Save conversation history
